# <?xml version="1.0" encoding="UTF-8" ?>
#
# <launch>
#     <arg name="device" default="cuda" doc="Device to run tensor operations on: cpu or cuda"/>
#     <arg name="input_img_topic" default="robot_data/rgb/compressed"/>
#     <arg name="data_sequence" default="00000" doc="Sequence name from Rellis-3D dataset"/>
#     <arg name="rviz" default="true"/>
#     <arg name="model_name" default="smp" doc="Currently supported options: ['hrnet', 'smp']"/>
# <!--    <arg name="smp_weights" default="PSPNet_resnext50_32x4d_704x960_lr0.0001_bs6_epoch18_Rellis3D_iou_0.73.pth"-->
# <!--         doc="SMP model weights (file name stored in 'config/weights/smp/' folder)"/>-->
# <!--    <arg name="smp_weights" default="Unet_resnet34_1184x1920_lr0.0001_bs1_epoch2_Rellis3DImages_iou_0.77.pth"/>-->
#     <arg name="smp_weights" default="PSPNet_resnext50_32x4d_1184x1920_lr0.0001_bs1_epoch6_Rellis3DImages_iou_0.68.pth"/>
#     <arg name="hrnet_weights" default="seg_hrnet_ocr_w48_train_512x1024_sgd_lr1e-2_wd5e-4_bs_12_epoch484/best.pth"
#          doc="SMP model weights (file name stored in 'config/weights/' folder)"/>
#
#     <include file="$(dirname)/robot_data.launch">
#         <arg name="data_sequence" value="$(arg data_sequence)"/>
#     </include>
#
#     <node name="segmentation_inference" pkg="traversability_estimation" type="segmentation_inference" output="screen">
#         <rosparam subst_value="true">
#             model_name: $(arg model_name)
#             smp_weights: $(arg smp_weights)
#             hrnet_weights: $(arg hrnet_weights)
#             device: $(arg device)
#             num_cameras: 1
#             image_transport: 'compressed'
#             legend: false
#             max_age: 1.0
#             input_scale: 0.5
#             traversability_labels: false
#         </rosparam>
#         <remap from="input_0/image" to="/robot_data/rgb"/>
#         <remap from="input_0/image/compressed" to="/robot_data/rgb/compressed"/>
#         <remap from="input_0/camera_info" to="/robot_data/camera_info"/>
#
#         <remap from="output_0/semseg" to="segmentation_inference/semantic_segmentation"/>
#         <remap from="output_0/semseg/compressed" to="segmentation_inference/semantic_segmentation/compressed"/>
#         <remap from="output_0/camera_info" to="segmentation_inference/camera_info"/>
#     </node>
#
#     <!-- RVIZ -->
#     <node if="$(arg rviz)" name="rviz" pkg="rviz" type="rviz"
#           args="-d $(find traversability_estimation)/config/rviz/robot_data.rviz"/>
#
# </launch>

from launch import LaunchDescription
from launch.actions import GroupAction
from launch.actions import IncludeLaunchDescription
from launch.actions import DeclareLaunchArgument
from launch.actions import SetEnvironmentVariable
from launch.launch_description_sources import PythonLaunchDescriptionSource
from launch.substitutions import LaunchConfiguration
from launch.conditions import IfCondition, UnlessCondition, LaunchConfigurationEquals
from launch_ros.substitutions import FindPackageShare
from launch_ros.actions import Node
from launch_ros.actions import PushRosNamespace
from launch_ros.actions import SetParameter
from ament_index_python.packages import get_package_share_directory

def generate_launch_description():
    return LaunchDescription([
        DeclareLaunchArgument(name='device', default_value='cuda', description='Device to run tensor operations on: cpu or cuda'),
        DeclareLaunchArgument(name='input_img_topic', default_value='robot_data/rgb/compressed'),
        DeclareLaunchArgument(name='data_sequence', default_value='00000', description='Sequence name from Rellis-3D dataset'),
        DeclareLaunchArgument(name='rviz', default_value='true'),
        DeclareLaunchArgument(name='model_name', default_value='smp', description='Currently supported options: [\'hrnet\', \'smp\']'),
        DeclareLaunchArgument(name='smp_weights', default_value=
                              # 'PSPNet_resnext50_32x4d_1184x1920_lr0.0001_bs1_epoch6_Rellis3DImages_iou_0.68.pth'
                              # 'fcn_resnet50_lr_1e-05_bs_2_epoch_44_TraversabilityImages_iou_0.86.pth'
                              'Unet_resnet34_1184x1920_lr0.0001_bs1_epoch2_Rellis3DImages_iou_0.77.pth'
                              # 'DeepLabV3_resnet34_704x960_lr0.0001_bs6_epoch2_Rellis3D_iou_0.74.pth'
                              # 'PSPNet_resnext50_32x4d_704x960_lr0.0001_bs6_epoch18_Rellis3D_iou_0.73_legacy.pth'
                              # 'se_resnext50_32x4d_352x640_lr1e-4.pth'
                              # 'FPN_resnet34_704x960_lr0.0001_bs8_epoch19_Rellis3D_iou_0.74.pth'
                              ),
        DeclareLaunchArgument(name='hrnet_weights', default_value=
                              'seg_hrnet_ocr_w48_train_512x1024_sgd_lr1e-2_wd5e-4_bs_12_epoch484/best.pth',
                              # 'hrnetv2_w48_imagenet_pretrained.pth',
                              description='SMP model weights (file name stored in \'config/weights/\' folder)'),

        # SetParameter(name='use_sim_time', value=True),
        # 'use_sim_time' will be set on all nodes following the line above

        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                [FindPackageShare('traversability_estimation'), '/launch/robot_data.launch.py'],
            ),
            launch_arguments={
                'data_sequence': LaunchConfiguration('data_sequence'),
                'pose_step': '1',
                'period': '0.1',
            }.items(),
        ),

        Node(
            name='segmentation_inference',
            package='traversability_estimation',
            executable='segmentation_inference',
            output='screen',
            parameters=[{
                'model_name': LaunchConfiguration('model_name'),
                'smp_weights': LaunchConfiguration('smp_weights'),
                'hrnet_weights': LaunchConfiguration('hrnet_weights'),
                'device': LaunchConfiguration('device'),
                'num_cameras': 1,
                'image_transport': 'compressed',
                'legend': False,
                'max_age': 1.0,
                'input_scale': 0.5,
                'traversability_labels': False,
            }],
            remappings=[
                ('input_0/image', '/robot_data/rgb'),
                ('input_0/image/compressed', '/robot_data/rgb/compressed'),
                ('input_0/camera_info', '/robot_data/camera_info'),
                ('output_0/semseg', 'segmentation_inference/semantic_segmentation'),
                ('output_0/semseg/compressed', 'segmentation_inference/semantic_segmentation/compressed'),
                ('output_0/camera_info', 'segmentation_inference/camera_info'),
            ],
        ),

        # RVIZ
        Node(
            condition=IfCondition(LaunchConfiguration('rviz')),
            name='rviz',
            package='rviz2',
            executable='rviz2',
            arguments=['-d', FindPackageShare('traversability_estimation'), '/config/rviz/robot_data.rviz'],
        ),
    ])
